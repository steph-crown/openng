...
export function FrameworkCardsGrid() {
  return (
    <section className="mt-[22px] grid grid-cols-2 gap-5 max-[1000px]:grid-cols-1">
      <article className="grid min-h-[320px] content-start gap-4 rounded-(--radius-xl) border border-(--color-border) bg-(--color-surface) p-6">
        ...
        <div className="inline-flex items-center gap-1 self-end">
          <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-(--color-brand) text-xs font-bold text-(--color-brand-foreground)">
            JS
          </span>
          <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-(--color-brand) text-xs font-bold text-(--color-brand-foreground)">
            Py
      ...
      <article className="grid min-h-[320px] content-start gap-4 rounded-(--radius-xl) border border-(--color-border) bg-(--color-surface) p-6">
        ...
        <ul className="m-0 list-none overflow-hidden rounded-[10px] border border-(--color-border) p-0">
          {frameworkPackages.map((item) => (
            <li
              key={item.name}
              className="grid grid-cols-[1fr_1fr] gap-2.5 border-b border-(--color-border) px-3 py-2.5 text-[13px] last:border-b-0 max-[720px]:grid-cols-1"
            >
              <strong>{item.name}</strong>
              <span className="text-(--color-muted)">{item.description}</span>
            </li>
          ))}
        </ul>
      </article>
      <article className="grid min-h-[320px] content-start gap-4 rounded-(--radius-xl) border border-(--color-border) bg-(--color-surface) p-6">
        ...
        <code className="block rounded-[10px] border border-(--color-border) bg-[color-mix(in_oklab,var(--color-surface-strong)_80%,transparent)] px-3 py-2.5 font-(--font-mono) text-[13px]">
          Use cases: reminders, pricing, address validation, facility lookup
        </code>
      </article>
      <article className="min-h-[320px] overflow-hidden rounded-(--radius-xl) border border-(--color-border) bg-(--color-surface)">
        <img
